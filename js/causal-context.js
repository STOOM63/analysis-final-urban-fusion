...
AU.causalContext = (() => {
  ...
  function testFinding(model,f){
    ...
    const urbanScore=Math.max(workScore,bestAlternative?.strength||0);
    const source=model.contextCorrelation?.source;
    const freshness=source?.stale?55:source?.apiOk?100:75;
    let score=clamp(concentrationScore*0.23+excessScore*0.20+urbanScore*0.40+sample*0.08+freshness*0.09,0,100);
    if(!best && !bestAlternative) score=Math.min(score,34);
    else if(!best && bestAlternative && bestAlternative.strength<58) score=Math.min(score,52);
    if(top?.geo?.impactScore>=70) score=clamp(score+6,0,100);
    const status=score>=78?'strong':score>=58?'moderate':score>=36?'weak':'none';
    const label={strong:'Contexte urbain fortement compatible',moderate:'Contexte urbain compatible',weak:'Contexte urbain faible',none:'Aucun facteur externe convaincant'}[status];
    const chain=[
      `Constat : ${f.title}`,
      ...
      best?`Travaux les plus compatibles : ${best.place||best.sector} — ${best.severityReason} — affinité géographique ${best.zoneAffinity}/100 — score événement ${best.compatibility}/100.`:bestAlternative?`Facteur externe dominant : ${bestAlternative.label} — signal ${Math.round(bestAlternative.strength)}/100 — ${bestAlternative.evidence}`:'Aucun facteur urbain externe suffisamment compatible trouvé.'
    ];
    ...
    const summary=status==='strong'
      ? `Le contexte urbain externe constitue une explication fortement compatible pour ce constat, surtout via ${top?.zone||'la zone dominante'}. Le moteur a comparé travaux, mobilité, transports, stationnement, événements et météo ; cela reste probabiliste.`
      : status==='moderate'
        ? `Un facteur urbain externe est plausible mais n'explique pas seul le constat. Les causes métier et saisonnières restent testées en parallèle.`
        : status==='weak'
          ? `Quelques coïncidences externes existent, mais elles sont trop faibles pour être retenues comme explication principale.`
          : `Le moteur a testé travaux, mobilité, T2C, stationnement, événements et météo sans trouver de facteur externe suffisamment convaincant.`;
    return {tested:true,status,score:Math.round(score),label,summary,scope:scope.filterLabel,targetDate,topZone:top?.zone||null,concentration:geo.concentration,zoneExcess:top?.excess??null,chain,works,alternatives,dominantExternal:best?{key:'works',label:'Travaux / accessibilité',strength:workScore,evidence:best.place||best.sector}:bestAlternative,sourceFreshness:freshness};
  }
  ...
  function analyze(model,intel){
    ...
    const actions=top.slice(0,10).map(x=>({
      action:x.status==='strong'?`Suivre automatiquement le couple « ${x.title} » ↔ ${x.topZone||'zone concernée'} et le facteur externe dominant à chaque nouvel import.`:`Maintenir le contexte urbain comme hypothèse secondaire pour « ${x.title} » et tester les causes concurrentes.`,
  ...
  function apply(model){
    ...
    if(intel){
      ...
      if(c.strong||c.moderate){
        const line=`Couplage urbain Clermont exécuté sur ${c.tested} diagnostic(s) : ${c.strong} compatibilité(s) forte(s), ${c.moderate} moyenne(s). Travaux, T2C, stationnement, événements, météo et mobilité sont testés sans confondre corrélation et causalité.`;
  ...
  function answer(model){
    ...
    return {
      title:'Contexte urbain, mobilité et causes externes',
      intro:`${c.tested} diagnostic(s) métier ont été automatiquement testés contre le contexte urbain Clermont : ${c.strong} compatibilité(s) forte(s), ${c.moderate} moyenne(s).`,
