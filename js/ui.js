...
AU.ui = (() => {
  ...
  function causalBadge(c){
    ...
    return `<span class="causal-badge ${cls}">Contexte urbain · ${U().escapeHtml(c.label||'testé')} ${c.score?`· ${U().integer(c.score)}/100`:''}</span>`;
  }
  ...
  function causalEvidence(c){
    ...
    return `<div class="causal-box ${U().escapeHtml(c.status||'none')}"><div class="causal-head">${causalBadge(c)}</div><p>${U().escapeHtml(c.summary||'')}</p>${chain?`<h4>Chaîne explicative automatique</h4><ol>${chain}</ol>`:''}${works?`<h4>Travaux publics testés</h4><ul>${works}</ul>`:''}${alternatives?`<h4>Facteurs alternatifs / concurrents testés</h4><ul>${alternatives}</ul>`:''}</div>`;
  }
  ...
  function geoView(model, root) {
    ...
    root.innerHTML=viewHeader('Geo Intelligence · Clermont',`Radar automatique clients → zones → CA → visites → contexte urbain · couverture ${U().percent(Number(g.coverage)||0)}`)+
  ...
  function contextView(model,root){
    ...
    if(!c){root.innerHTML=viewHeader('Contexte causal','Couplage automatique diagnostics ↔ travaux ↔ T2C ↔ parking ↔ événements ↔ météo ↔ mobilité')+'<section class="panel"><div class="empty-mini">Le moteur contextuel n’a pas encore été exécuté.</div></section>';return;}
    const top=c.top||[]; const events=c.events||[];
    root.innerHTML=viewHeader('Contexte causal Clermont','Chaque diagnostic métier est automatiquement testé contre les zones, travaux, T2C, stationnement, événements, météo, mobilité, saisonnalité et causes métier concurrentes',qualityPill(c.strong?'estimate':'calculated',`${c.tested} tests exécutés`))+
  ...
  function render(view,model,root){
    ...
    try{fn(model,root);root.dataset.viewState='ready';delete root.dataset.viewError;}
    catch(err){
      console.error(`Analysis Ultimate render error [${view}]`,err);root.dataset.viewState='error';root.dataset.viewError=String(err?.message||err);
